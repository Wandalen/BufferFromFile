...
function onSuiteBegin()
{
  ...
  context.suiteTempPath = _.path.tempOpen( _.path.join( __dirname, '..' ), 'BufferFromFile' );
...
function readOnlyBuffer( test )
{
  ...
  let program1Path = a.program({ entry : program1, locals : _.props.extend( null, locals ) }).filePath/*programPath*/;
  let program2Path = a.program({ entry : program2, locals : _.props.extend( null, locals ) }).filePath/*programPath*/;
...
function ipc( test )
{
  ...
  let program1Path = a.program({ entry : program1, locals }).filePath/*programPath*/;
